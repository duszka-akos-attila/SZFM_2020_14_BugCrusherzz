...
import com.tracky2.data.dao.ExpenseDao_Impl;
import com.tracky2.data.dao.GroupDao;
import com.tracky2.data.dao.GroupDao_Impl;
import com.tracky2.data.dao.IncomeDao;
import com.tracky2.data.dao.IncomeDao_Impl;
import com.tracky2.data.dao.TemplateDao;
import com.tracky2.data.dao.TemplateDao_Impl;
  ...
  protected SupportSQLiteOpenHelper createOpenHelper(DatabaseConfiguration configuration) {
    final SupportSQLiteOpenHelper.Callback _openCallback = new RoomOpenHelper(configuration, new RoomOpenHelper.Delegate(1) {
      ...
      public void createAllTables(SupportSQLiteDatabase _db) {
        _db.execSQL("CREATE TABLE IF NOT EXISTS `Expense` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `amount` INTEGER NOT NULL, `description` TEXT, `groupId` INTEGER NOT NULL, `date` INTEGER)");
        _db.execSQL("CREATE TABLE IF NOT EXISTS `Income` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `amount` INTEGER NOT NULL, `description` TEXT, `date` INTEGER)");
        _db.execSQL("CREATE TABLE IF NOT EXISTS `Group` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `name` TEXT, `color` INTEGER NOT NULL)");
        _db.execSQL("CREATE TABLE IF NOT EXISTS `Template` (`isIncome` INTEGER NOT NULL, `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `amount` INTEGER NOT NULL, `description` TEXT, `groupId` INTEGER NOT NULL)");
        _db.execSQL("CREATE TABLE IF NOT EXISTS room_master_table (id INTEGER PRIMARY KEY,identity_hash TEXT)");
        _db.execSQL("INSERT OR REPLACE INTO room_master_table (id,identity_hash) VALUES(42, 'b70eb9cc0ce42f72888ce7390c193666')");
      }
      ...
      protected RoomOpenHelper.ValidationResult onValidateSchema(SupportSQLiteDatabase _db) {
        ...
        if (! _infoExpense.equals(_existingExpense)) {
          return new RoomOpenHelper.ValidationResult(false, "Expense(com.tracky2.data.Expense).\n"
                  + " Expected:\n" + _infoExpense + "\n"
                  + " Found:\n" + _existingExpense);
        }
        ...
        if (! _infoIncome.equals(_existingIncome)) {
          return new RoomOpenHelper.ValidationResult(false, "Income(com.tracky2.data.Income).\n"
                  + " Expected:\n" + _infoIncome + "\n"
                  + " Found:\n" + _existingIncome);
        }
        ...
        if (! _infoGroup.equals(_existingGroup)) {
          return new RoomOpenHelper.ValidationResult(false, "Group(com.tracky2.data.Group).\n"
                  + " Expected:\n" + _infoGroup + "\n"
                  + " Found:\n" + _existingGroup);
        }
        ...
        if (! _infoTemplate.equals(_existingTemplate)) {
          return new RoomOpenHelper.ValidationResult(false, "Template(com.tracky2.data.Template).\n"
                  + " Expected:\n" + _infoTemplate + "\n"
                  + " Found:\n" + _existingTemplate);
        }
        return new RoomOpenHelper.ValidationResult(true, null);
      }
    }, "b70eb9cc0ce42f72888ce7390c193666", "ca25f7858053f0eb7bd220cceb339a33");
